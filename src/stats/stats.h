...
class Stats {
 public:
  ...
  std::atomic<int> reach_outbuf_limit_disconnections = {0};
  std::atomic<int> stat_evictedclients = {0};
  ...
  void IncrReachOutbufLimitDisconnections() {
    reach_outbuf_limit_disconnections.fetch_add(1, std::memory_order_relaxed);
  }
