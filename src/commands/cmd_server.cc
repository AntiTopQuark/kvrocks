 ...
 public:
  Status Execute(Server *srv, Connection *conn, std::string *output) override {
    ...
    if (args_.size() == 3 && sub_command == "get") {
      ...
      *output = s.IsOK() ? conn->SimpleString("OK") : redis::Error(s);
...
class CommandScan : public CommandScanBase {
 public:
  ...
  static std::string GenerateOutput(Server *srv, [[maybe_unused]] Connection *conn,
...
class CommandReset : public Commander {
 public:
  Status Execute(Server *srv, Connection *conn, std::string *output) override {
    ...
    *output = conn->SimpleString("RESET");
    return Status::OK();
  }
};
